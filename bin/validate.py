...
"""
@tools.gzips
def produce_gaf(dataset, source_gaf, ontology_graph, gpipath=None, paint=False, group="unknown", rule_metadata=None, goref_metadata=None, db_entities=None, group_idspace=None, format="gaf", suppress_rule_reporting_tags=[], annotation_inferences=None, mixin=False, rule_contexts=[]):
...
def mixin_a_dataset(valid_gaf, mixin_metadata_list, group_id, dataset, target, ontology, gpipath=None, base_download_url=None, replace_existing_files=True, rule_contexts=[]):
    ...
    for mixin_metadata in mixin_metadata_list:
        ...
        if mixin_src is not None:
            ...
            mixin_gaf = produce_gaf(mixin_dataset_id, mixin_src, ontology, gpipath=gpipath, paint=True, group=mixin_metadata["id"], format=format, mixin=True, rule_contexts=rule_contexts)[0]
